...
class Controller:
    ...
    def run(self):
        ...
        self.obs[6 + num_actions * 2 : 21 + num_actions * 2] = np.clip(self.action, -50, 50) # self.action
        # Get the action from the policy network
        self.obs[21 + num_actions * 2:24 + num_actions * 2] = self.cmd * self.config.cmd_scale * self.config.max_cmd
        obs_tensor = torch.from_numpy(self.obs[:82]).unsqueeze(0)
