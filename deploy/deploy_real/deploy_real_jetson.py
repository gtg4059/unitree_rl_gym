...
class Controller:
    ...
    def run(self):
        ...
        # self.obs_t = torch.as_tensor(self.obs).unsqueeze(0) #torch.from_numpy(self.obs).float().unsqueeze(0)
        # with torch.no_grad():
        #     self.action = self.policy_run(self.obs_t).squeeze(0).cpu().numpy()

        # self.action = self.policy_run(obs_tensor).detach().numpy().squeeze()

        middle = time.time()-time_niddle2
        time_niddle3 = time.time()
